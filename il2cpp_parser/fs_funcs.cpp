...
void fs::F_FilesInThisDirectory(std::string directory, std::vector<std::string_view>* out)
{
	out->clear();
	out->resize(1);
...
std::string fs::_GetLastError()
{
	...
	printf("IO_WriteData failed with: %s\n", messageBuffer);
...
void fs::F_SyntaxError(const char* msg, ...)
{
	...
	printf("Syntax error on line [%i, %i] with reason: [%s]\n", file.lines_read, file.current_column, v2);
